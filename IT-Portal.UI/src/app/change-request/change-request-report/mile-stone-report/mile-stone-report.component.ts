...
import { MatPaginator, PageEvent } from '@angular/material/paginator';import { environment } from '@environments/environment';
...
@Component({
  selector: 'app-mile-stone-report',
  templateUrl: './mile-stone-report.component.html',
  styleUrl: './mile-stone-report.component.css'
})
export class MileStoneReportComponent {
  ...
  getviewcrdata() {
    ...
    const apiUrls = this.apiurl + '/ViewChangeRequest/getMileStoneCR';
    const requestBody = {};
    const httpOptions = {
  ...
  exportToPDF() {
    ...
    (doc as any).autoTable({
        ...
        body: Array.from(tableElement.querySelectorAll('tbody tr')).map(row => {
            return Array.from(row.querySelectorAll('td')).slice(0, 5).map(td => td.textContent || ''); // Ensure not null
        })
    });

    doc.save('MileStoneReport.pdf');
  }
