...
@Component({
  ...
  styleUrls: ['./asset-master.component.css', '../assetmodule.css', '/src/app/service-module/servicemodule.css'],
  animations: [
...
export class AssetMasterComponent {
  ...
  getAssetData() {
    this.httpSer.httpGet('/AssetRequest/GetAssets').subscribe(
      (response: any) => {
        this.assetList = response?.data;
        this.toggleMyServiceTable('S',true,true)
  ...
  navigateToEdit(assetID: any) {
    this.route.navigate(['/assets/itasset'], { queryParams: { assetid: assetID, isEngineer: this.isAssetActive=='S' } });
  }
  ...
  toggleMyServiceTable(myService: any, first = false, isOpenSer = true) {
    ...
    this.pageIndex = 0
    if (first) {
      this.asetListShow = (this.assetList?.filter((m: any) => {
      ...
      this.pendingAssetListShow = this.assetList?.filter((m: any) => {
    ...
    if (this.isAssetActive == 'S') {
      this.filteredList = this.assetList?.filter((m: any) => {
        return (m.hodEmpNo?.toString() == this.user?.empData?.employeeNo || m.rpmEmpNo == this.user?.empData?.employeeNo || this.user?.supportTeamAssignList[0]?.isSuperAdmin
          || this.user?.supportTeamAssignList?.map((a: any) => a.supportId).includes(m.supportId)) && m.status?.trim() != 'Draft'
      })
  ...
  delete(srId: any) {
    this.httpSer.httpGet('/AssetRequest/deleteAssetbyID', { ID:  srId}).subscribe((res: any) => {
      alert('Service Deleted Successfully.')
      this.assetList = this.assetList.filter((m: any) => m.itassetId != srId)
