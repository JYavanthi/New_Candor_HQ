...
@Component({
  ...
  templateUrl: './issue-new-issue.component.html',
  styleUrl: './issue-new-issue.component.css'
})
export class IssueNewIssueComponent {
  ...
  constructor(private http: HttpClient, private datepipe: DatePipe, private employeeInfo: GetEmployeeInfoService, private userInfo: UserInfoSerService, private route: Router, private routeservice: PasscrdataService, private fb: FormBuilder,
    public httpSer: HttpServiceService) {
    this.routeservice.getsupportteam();
    this.supportid = this.routeservice.supporterID;
    this.user = this.userInfo.getUser();
    this.adminEmpId = this.user.supportTeamAssignList.filter((m: any) => m.supportId == 3 && m.isAdmin && !m.isSuperAdmin && m.isActive).length > 0 ? Number(this.user.empData.employeeNo) : 0
  ...
  getInputDatas() {
    ...
    this.Email = this.supportteams[0]?.officialEmailId;
    this.ContactNo = this.supportteams[0]?.mobileNo;
  ...
  getOtherInputDatas() {
    ...
    this.EmailOthers = this.supportteamsforOthers[0]?.officialEmailId;
    this.ContactNoOthers = this.supportteamsforOthers[0]?.mobileNo;
  ...
  getcategory() {
    ...
    this.http.get(apiUrls).subscribe(
      (response: any) => {
        this.categorydata = response.filter((row: any) => row.isActive);
      },
      (error) => {
        console.error("Post failed", error)
      }
  ...
  getassets() {
    this.assignToList = [];
    if (this.selectedOption == 'self' && this.EmployeeID != '') {
      this.OnlyAsset(this.EmployeeID);
    }
    if (this.selectedOption == 'others' && this.EmployeeIDOthers != '') {
      this.OnlyAsset(this.EmployeeIDOthers);
    }
    if (this.selectedOption == 'guest' && this.GuestEmployeeID != '') {
      this.OnlyAsset(this.GuestEmployeeID);
    }
    if (this.categoryId != '' && this.categoryTypId != '') {
      this.getAssignTo();
    }
  }
  ...
  OnlyAsset(emp: any) {
    ...
    this.http.get(apiUrls).subscribe(
      (response: any) => {
        ...
        const assetlist = response.filter((item: any) => item.empNo == emp);
    ...
    if (this.categoryId == '') {
      ...
      try {
        ...
        if (this.supportteam) {
          ...
          const isExecutiveRole = isHelpdeskSupport.filter((row: any) => row.isSuperAdmin || row.isadmin || row.issupportengineer)
          const ServiceCategory = this.categorydata.filter((col: any) => col.itcategoryId == this.categoryId)
          const assignServiceCat = isExecutiveRole.filter((item: any) => item.isSuperAdmin || item.categoryId == this.categoryId)
  ...
  ExecuteFunc(status: any) {
    ...
    const requestBody =
    {
      ...
      "issueRaiseFor": Number(this.EmployeeIDOthers) || 0,
      "issueForGuest": "",
      ...
      "proposedResolutionOn": this.prDate,
    ...
    this.httpSer.httpPost(apiUrl, requestBody).subscribe(
      (response: any) => {
        ...
        if (response.type == "S" && status == "Draft") {
          ...
          this.route.navigate(['/report_issue']);
          this.getIssueList().then(() => {
            const issueCode = this.issueList.filter((item: any) => parseInt(item.issueId) == parseInt(response.data.issueID))[0].issueCode;
            this.sendemailfrom(getStatus, formattedOutput, issueCode)
          })
        }
        else {
  ...
  issueList: any[] = [];
  async getIssueList() {
    const apiUrls = '/IssueList/Getissuelist';
    try {
      const response = await this.httpSer.httpGet(apiUrls).toPromise();
      if (response) {
  ...
  //   const apiUrl = this.apiurl + '/issueResolutionAttach/saveDocument';
  ...
  filterItems() {
    ...
    const isUserItems = this.supportnames.filter(item =>
      item.employeeId.toUpperCase().toString().includes(filter) || item.employeeName.toUpperCase().toString().includes(filter)
    );
    this.dropdownItems = isUserItems.filter((row: any) => row.employeeId != this.user.empData.employeeNo)
  ...
  async fetchAllItems() {
    try {
      ...
      if (this.selectedOption == "others") {
        this.supportteamsforOthers = this.allEmpList.filter((row: any) => row.employeeId == parseInt(this.issueRaiseFor.trim()));
      }
      if (this.selectedOption == "guest") {
  ...
  */
  ...
  sendemailfrom(status: any, IssueDesc: any, getIssueCode: any) {
    ...
    const requestBody = {
      ...
      "subject": `Unnati: IT ISSUES: ${getIssueCode}: Logged`,
