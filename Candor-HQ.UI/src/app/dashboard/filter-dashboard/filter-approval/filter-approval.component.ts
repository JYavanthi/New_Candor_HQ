...
@Component({
  ...
  styleUrl: './filter-approval.component.css'
})
export class FilterApprovalComponent {
  ...
  downloadExcel(): void {
    ...
    XLSX.writeFile(wb, 'Approval_excel.xlsx');
  }

 
  filteredApprovedtData: any[] = [];
  ...
  filterData() {
    // Filter Draft data based on the received plantId
    this.filteredApprovedtData = this.viewchangerequest.filter(item => item.plantId === this.plantId && item.status === 'Approved');
  }
