namespace IT_Portal.Domain.IT_Models;

public partial class EmpOfficialInfo
{
    public int? Id { get; set; }

    public int? Pernr { get; set; }

    public string? EmpFullname { get; set; }

    public string? Sex { get; set; }

    public string? Werks { get; set; }

    public string? Bukrs { get; set; }

    public int? Dptid { get; set; }

    public int? Dsgid { get; set; }

    public string? Appmgr { get; set; }

    public string? Rptmgr { get; set; }

    public string? Locid { get; set; }

    public string? Land1 { get; set; }

    public int? State { get; set; }

    public string? TelNo { get; set; }

    public int? TelExtens { get; set; }

    public int? IpPhone { get; set; }

    public string? EmailId { get; set; }

    public string? Room { get; set; }

    public string? Floor { get; set; }

    public string? Building { get; set; }

    public DateOnly? Doj { get; set; }

    public DateOnly? Doc { get; set; }

    public DateOnly? Pdol { get; set; }

    public DateOnly? Dol { get; set; }

    public string? Esi { get; set; }

    public string? Esino { get; set; }

    public string? Pf { get; set; }

    public string? Pfno { get; set; }

    public string? Pt { get; set; }

    public string? It { get; set; }

    public string? Panno { get; set; }

    public string? Bonus { get; set; }

    public string? Leaves { get; set; }

    public string? PayGroup { get; set; }

    public string? Staffcat { get; set; }

    public string? Grdid { get; set; }

    public string? PaymentMethod { get; set; }

    public string? Waers { get; set; }

    public string? Bacctyp { get; set; }

    public string? Baccno { get; set; }

    public int? Bankid { get; set; }

    public string? Branch { get; set; }

    public string? IfscCode { get; set; }

    public string? MicrCode { get; set; }

    public DateOnly? Dob { get; set; }

    public string? BoardLine { get; set; }

    public string? Title { get; set; }

    public string? Uanno { get; set; }

    public bool? Active { get; set; }

    public string? SapStatus { get; set; }

    public DateTime? SapApprovedDate { get; set; }

    public string? SapMessage { get; set; }

    public string? ShiftCode { get; set; }

    public string? RuleCode { get; set; }

    public int? SwipeCount { get; set; }

    public int? WorkId { get; set; }

    public DateTime? CreatedDate { get; set; }

    public int? ReportingGrp { get; set; }

    public int? Sdptid { get; set; }

    public int? SwipeNo { get; set; }

    public bool? IsHod { get; set; }

    public int? ConId { get; set; }

    public double? ConSal { get; set; }

    public string? AadharNo { get; set; }

    public string? Dstatus { get; set; }

    public int? Sdptid1 { get; set; }

    public string? Wing { get; set; }
}
