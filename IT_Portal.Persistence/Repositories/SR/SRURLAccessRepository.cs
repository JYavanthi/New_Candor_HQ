...
{
    public class SRURLAccessRepository : ISRURLAccess
    {
        private readonly MicroLabsDevContext _context;

        public SRURLAccessRepository(MicroLabsDevContext context)
        {
        ...
        public async Task<CommonRsult> GetURLAccess()
        {
            ...
            try
            {
                var value = _context.Srurlaccesses;
        ...
        public async Task<CommonRsult> GetURLAccessValue(int srid)
        {
            ...
            try
            {
                var value = _context.Srurlaccesses.Where(m => m.Srid == srid);
                var a = from m in value
                        join employee in _context.Employees on m.CreatedBy.ToString() equals employee.EmployeeNo into emp
                        from employee in emp.DefaultIfEmpty()
        ...
        public async Task<CommonRsult> PostURLAccess(SRURLAccess sRURL)
        {
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand("IT.sp_SRURLAccess", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@Flag", sRURL.Flag);
                    cmd.Parameters.AddWithValue("@SRID", sRURL.SRID);
                    cmd.Parameters.AddWithValue("@SRURLID", sRURL.SRURLID);
                    ...
                    cmd.Parameters.AddWithValue("@URLRequired", sRURL.URLRequired);
