...
//                    cmd.Parameters.AddWithValue("@ClassificationID", spChangerequest.ClassificationId);
...
namespace IT_Portal.Persistence.Repositories
{
    public class ChangeRequestRepository : IChangeRequest
    {
        ...
        public async Task<CommonRsult> InsertChangeRequest(ChangeRequestSP spChangerequest)
        {
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand("IT.Sp_ChangeRequest", con))
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        if (spChangerequest.Type == "I")
                        {
                            cridNew = _context.ChangeRequests
                                .OrderByDescending(m => m.Itcrid)
                                .Select(m => m.Itcrid)
                                .FirstOrDefault();

                            if (spChangerequest.AttachmentIds != null)
                            {
                                foreach (var item in spChangerequest.AttachmentIds)
                                {
                                    var attchObj = _context.AttachFiles
                                        .FirstOrDefault(m => m.AttachId == item);
                    ...
                    {
                        result.Type = "E";
                        result.Message = "Insert failed - No rows affected";
                    }
                }
            }
            catch (Exception ex)
            {
                result.Type = "E";
                result.Message = ex.Message; // Show real error for debugging
            }
